...
function ProposalCard({ proposal, index }: { proposal: Proposal; index: number }) {
  ...
  return (
    <div
      className="group rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur-xl p-6 transition-all duration-300 hover:bg-white/[0.07] hover:border-[#8a00ff]/30 hover:shadow-[0_8px_40px_rgba(138,0,255,0.12)]"
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className={`font-ticker text-[10px] px-2.5 py-0.5 rounded-full border ${catStyle.pill}`}>
            {proposal.category}
          </span>
          {isUrgent && (
      ...
      <div className="mb-3">
        ...
        <div className="flex justify-between">
          <span className="font-ticker text-[10px] text-[#b84dff]">
            For {forPct.toFixed(1)}% · {(proposal.votesFor / 1_000_000).toFixed(2)}M
          </span>
          <span className="font-ticker text-[10px] text-red-400">
            {(proposal.votesAgainst / 1_000_000).toFixed(2)}M · {(100 - forPct).toFixed(1)}% Against
          </span>
        </div>
      </div>
      ...
      {/* Proposer */}
      <p className="font-ticker text-[10px] text-white/20 mt-3">
...
function VotingPowerPanel() {
  ...
  return (
    <div
      className={`rounded-3xl border backdrop-blur-xl p-6 relative overflow-hidden transition-all duration-500 ${
        canPropose
          ? "border-[#8a00ff]/40 bg-[#8a00ff]/[0.06] shadow-[0_0_40px_rgba(138,0,255,0.15)]"
...
export default function GovernanceHubPage() {
  ...
  const filtered =
    filter === "all" ? PROPOSALS : PROPOSALS.filter((p) => p.category === filter);
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="font-body mt-3 text-white/60 max-w-lg text-sm leading-relaxed">
              Shape the protocol. Vote on parameter changes, allocate treasury funds,
              and guide the future of StellarStream.
            </p>
          </div>
            ...
            <div className="divide-y divide-white/[0.04]">
              {PAST_VOTES.map(({ id, vote, outcome }) => (
                ...
                >
                  ...
                  <span
                    className={`font-ticker text-[10px] ${
                      vote === "For" ? "text-[#b84dff]" : "text-red-400"
                    }`}
                  >
                    {vote}
                  </span>
                  <span
                    className={`font-ticker text-[10px] ${
                      outcome === "Passed" ? "text-emerald-400" : "text-white/25"
                    }`}
                  >
                    {outcome}
                  </span>
